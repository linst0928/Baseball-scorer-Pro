...
function TeamPill({ team, side }: { team: Team; side: TeamSide }) {
  return (
    <View style={[styles.teamPill, side === "home" ? styles.homePill : styles.awayPill, { backgroundColor: teamSurfaceColor(team, side) }]}>
      <Text style={styles.teamPillLabel}>{side === "home" ? "主場" : "客場"}</Text>
...
function App() {
  ...
  const [newGameForm, setNewGameForm] = useState<NewGameForm>({ name: "", competition: "", ageGroup: "U12", venue: "", date: new Date().toISOString().slice(0, 10), weather: "sunny", awayTeamId: "team-away", homeTeamId: "team-home", awayRegisteredPlayerIds: [], homeRegisteredPlayerIds: [], pitchLimitThresholds: [50, 70, 85], maxInnings: 9 });
  ...
  return (
    <View style={styles.pageGap}>
      ...
      <View style={styles.recentSearchRow}>
        <TextInput value={recentSearch.dateFrom} onChangeText={(dateFrom) => setRecentSearch((current) => ({ ...current, dateFrom }))} placeholder="起始日期 YYYY-MM-DD" placeholderTextColor={BRAND.muted} style={styles.recentDateInput} returnKeyType="done" />
        <Text style={styles.recentSearchSeparator}>至</Text>
        <TextInput value={recentSearch.dateTo} onChangeText={(dateTo) => setRecentSearch((current) => ({ ...current, dateTo }))} placeholder="結束日期 YYYY-MM-DD" placeholderTextColor={BRAND.muted} style={styles.recentDateInput} returnKeyType="done" />
...
function RecordView({ game, games, away, home, myTeam, mySide, battingTeam, pitchingTeam, batter, pitcher, teamPerformance, awayPerformance, homePerformance, pitchDraft, fieldingPosition, selectedResult, selectedPitchType, selectedPitchZone, recordColumnDraft, recentEvents, canUndo, onUndo, onRunnerAction, onSubstitute, onSpecialEvent, onStart, onPitch, onSelectResult, onClearSelectedResult, onOutcome, onPosition, onRecordColumnChange, onPitchType, onPitchZone, onEdit, onFinish, onOpenSymbolReference, onOpenSymbolHelp, onUpdatePlayer }: { game: Game; games: Game[]; away: Team; home: Team; myTeam: Team; mySide: TeamSide | null; battingTeam: Team; pitchingTeam: Team; batter?: Player; pitcher?: Player; teamPerformance: ReturnType<typeof getTeamPerformanceSummary> | null; awayPerformance: ReturnType<typeof getTeamPerformanceSummary> | null; homePerformance: ReturnType<typeof getTeamPerformanceSummary> | null; pitchDraft: PitchDraft; fieldingPosition: string; selectedResult: AtBatResult | null; selectedPitchType: PitchType; selectedPitchZone: PitchLocation["zone"]; recordColumnDraft: RecordColumn; recentEvents: Array<{ id: string; batterName: string; notation: string; inning: number; half: TeamSide; result: AtBatResult; pitches: { balls: number; strikes: number; total: number } }>; canUndo: boolean; onUndo: () => void; onRunnerAction: (type: "SB" | "CS" | "ADV" | "WP" | "PB" | "BK", base?: 1 | 2 | 3, targetBase?: 2 | 3 | 4) => void; onSubstitute: (role?: "代打" | "代跑" | "換投" | "換守") => void; onSpecialEvent: () => void; onStart: () => void; onPitch: (kind: PitchOutcome) => void; onSelectResult: (result: AtBatResult) => void; onClearSelectedResult: () => void; onOutcome: (result: AtBatResult, recordColumnOverride?: RecordColumn) => void; onPosition: (position: string) => void; onRecordColumnChange: (value: RecordColumn) => void; onPitchType: (type: PitchType) => void; onPitchZone: (zone: PitchLocation["zone"]) => void; onEdit: () => void; onFinish: () => void; onOpenSymbolReference: () => void; onOpenSymbolHelp: (help: SymbolHelp) => void; onUpdatePlayer: (teamId: string, playerId: string, patch: Partial<Player>) => void }) {
  ...
  return (
    <View style={styles.pageGap}>
      ...
      <View style={styles.liveTeamIdentityRow}><View style={[styles.liveTeamIdentityCard, { backgroundColor: teamSurfaceColor(away, "away") }]}><Text style={styles.liveTeamIdentitySide}>客場／先攻</Text><TeamLogoName team={away} textStyle={styles.liveTeamIdentityName} logoSize={19} /></View><View style={[styles.liveTeamIdentityCard, styles.liveTeamIdentityCardHome, { backgroundColor: teamSurfaceColor(home, "home") }]}><Text style={styles.liveTeamIdentitySide}>主場／後攻</Text><TeamLogoName team={home} textStyle={styles.liveTeamIdentityName} logoSize={19} align="right" /></View></View>
      <ScoreBoard game={game} away={away} home={home} />
      {game.status === "setup" ? <View style={styles.setupCard}><Text style={styles.setupTitle}>球員名單與比賽資訊已就緒</Text><Text style={styles.setupText}>客場先攻，記錄員可以從第一球開始建立完整比賽紀錄。</Text><Button label="開始第一局" onPress={onStart} /></View> : null}
      ...
      <View style={styles.fiveGridWorkspace}>
        ...
        <View style={[styles.fiveGridColumn, styles.middleLiveStack]}>
          <View style={[styles.liveQuadrant, styles.middleAtBatPanel, { backgroundColor: teamSurfaceColor(battingTeam, game.half), borderColor: teamAccentColor(battingTeam, game.half) }]}>
            <LivePanelTitle number="3" title="本次打者 · 早稻田打席格" subtitle={`第 ${atBatOrder} 棒 · ${batter ? playerIdentityLabel(batter) : "待選打者"} · 與左下投球格連動`} />
            <View style={styles.currentAtBatWorkRow}>
              <View style={styles.atBatSummaryPanel}>
                <View style={styles.summaryModeToggle}>
                  <Pressable onPress={() => setSummaryMode("compact")} accessibilityRole="button" accessibilityState={{ selected: summaryMode === "compact" }} style={({ pressed }) => [styles.summaryModeButton, summaryMode === "compact" && styles.summaryModeButtonActive, pressed && styles.pressed]}><Text style={[styles.summaryModeButtonText, summaryMode === "compact" && styles.summaryModeButtonTextActive]}>精簡</Text></Pressable>
                  <Pressable onPress={() => setSummaryMode("detailed")} accessibilityRole="button" accessibilityState={{ selected: summaryMode === "detailed" }} style={({ pressed }) => [styles.summaryModeButton, summaryMode === "detailed" && styles.summaryModeButtonActive, pressed && styles.pressed]}><Text style={[styles.summaryModeButtonText, summaryMode === "detailed" && styles.summaryModeButtonTextActive]}>詳細</Text></Pressable>
                </View>
                <View style={styles.liveMatchupRow}><Text style={styles.liveMatchupPlayer}>{playerIdentityLabel(batter, "#— 尚未設定")}</Text><Text style={styles.liveMatchupVs}>對</Text><Text style={styles.liveMatchupPitcher}>{playerIdentityLabel(pitcher, "#— 尚未設定")}</Text></View>
                <View style={[styles.pitchLimitStrip, pitchWarningStyle, pitchWarningPulse && pitchLimitWarning.level !== "none" && styles.pitchLimitPulse]}>
                  <Text numberOfLines={1} style={pitchWarningTextStyle}>P {currentPitcherPitches} 球{pitchLimitWarning.nextThreshold ? ` · 下個 ${pitchLimitWarning.nextThreshold}` : " · 三段已達"}</Text>
                  <Text numberOfLines={1} style={pitchWarningTextStyle}>{pitchLimitWarning.level === "yellow" ? "距門檻 3 球" : pitchLimitWarning.level === "orange" ? "距門檻 2 球" : pitchLimitWarning.level === "red" ? "下一球達門檻" : "投球監測"}</Text>
                </View>
                {pitcherHistories.length > 0 ? <View style={styles.pitcherHistoryStrip}>{pitcherHistories.map((history) => { const player = playerById.get(history.pitcherId); return <Text key={history.pitcherId} numberOfLines={1} style={[styles.pitcherHistoryChip, history.pitcherId === pitcher?.id && styles.pitcherHistoryChipCurrent]}>{playerIdentityLabel(player, "#— 投手")} {history.pitches}球{history.reachedThresholds.length ? ` · 已達${history.reachedThresholds.join("/")}` : history.nextThreshold ? ` · 下${history.nextThreshold}` : ""}</Text>; })}</View> : null}
                {summaryMode === "detailed" ? <><View style={styles.countRow}><View style={styles.countBox}><Text style={styles.countValue}>{pitchDraft.balls}</Text><Text style={styles.countLabel}>壞球</Text></View><View style={styles.countBox}><Text style={[styles.countValue, styles.strikeValue]}>{pitchDraft.strikes}</Text><Text style={styles.countLabel}>好球</Text></View><View style={styles.countBox}><Text style={[styles.countValue, styles.outValue]}>{game.outs}</Text><Text style={styles.countLabel}>出局</Text></View><View style={styles.countBox}><Text style={[styles.countValue, styles.pitchValue]}>{pitchDraft.total}</Text><Text style={styles.countLabel}>球數</Text></View></View><Text style={styles.summaryDetailMeta}>第 {atBatOrder} 棒 · {game.inning} 局{game.half === "away" ? "上" : "下"}</Text></> : <View style={styles.summaryCompactCount}><Text style={styles.summaryCompactValue}>B {pitchDraft.balls} · S {pitchDraft.strikes} · O {game.outs}</Text><Text style={styles.summaryCompactMeta}>{pitchDraft.total} 球 · 第 {atBatOrder} 棒</Text></View>}
              </View>
                <View style={styles.atBatRecordPanel}><CurrentAtBatPanel game={game} pitchDraft={pitchDraft} batter={batter} completedAtBat={latestCompletedAtBat} completedBatter={latestCompletedBatter} selectedResult={selectedResult} fieldingPosition={fieldingPosition} recordColumn={recordColumnDraft} /></View>
            </View>
          </View>
...
function LiveInfieldPanel({ game, pitchDraft, batter, pitcher, battingPlayers, selectedResult, fieldingPosition, recordColumn, runnerActionRail }: { game: Game; pitchDraft: PitchDraft; batter?: Player; pitcher?: Player; battingPlayers: Player[]; selectedResult: AtBatResult | null; fieldingPosition: string; recordColumn: RecordColumn; runnerActionRail: ReactNode }) {
  ...
  return <View style={styles.liveInfieldGrid}>
    ...
    <View style={styles.liveInfieldWorkRow}>
      <ImageBackground source={require("../../assets/images/live-infield-background.jpg")} resizeMode="contain" imageStyle={styles.liveRunnerCrossBackground} style={styles.liveRunnerCross}>
        <View style={styles.liveRunnerTopSlot}><View style={[styles.liveRunnerScoreCell, game.runners.second && styles.liveRunnerScoreCellOccupied]}><WasedaBaseCell {...quadrants[2]} dense style={{ borderWidth: 0 }} /></View><View style={[styles.liveRunnerPlayerTag, game.runners.second && styles.liveRunnerPlayerTagOccupied]}><Text style={styles.liveRunnerPlayerTagText} numberOfLines={1}>{second}</Text></View></View>
        <View style={styles.liveRunnerMiddleSlot}>
          <View style={styles.liveRunnerSideSlot}><View style={[styles.liveRunnerScoreCell, game.runners.third && styles.liveRunnerScoreCellOccupied]}><WasedaBaseCell {...quadrants[3]} dense style={{ borderWidth: 0 }} /></View><View style={[styles.liveRunnerPlayerTag, game.runners.third && styles.liveRunnerPlayerTagOccupied]}><Text style={styles.liveRunnerPlayerTagText} numberOfLines={1}>{third}</Text></View></View>
          <View style={styles.liveRunnerMound}><Text style={styles.liveRunnerMoundText}>投</Text><Text style={styles.liveRunnerMoundText}>#{pitcher?.number ?? "—"}</Text><Text style={styles.liveRunnerMoundMeta}>{pitchDraft.total} 球</Text></View>
          <View style={styles.liveRunnerSideSlot}><View style={[styles.liveRunnerScoreCell, game.runners.first && styles.liveRunnerScoreCellOccupied]}><WasedaBaseCell {...quadrants[1]} dense style={{ borderWidth: 0 }} /></View><View style={[styles.liveRunnerPlayerTag, game.runners.first && styles.liveRunnerPlayerTagOccupied]}><Text style={styles.liveRunnerPlayerTagText} numberOfLines={1}>{first}</Text></View></View>
        </View>
        <View style={styles.liveRunnerBottomSlot}><View style={styles.liveRunnerScoreCell}><WasedaBaseCell {...quadrants[0]} dense style={{ borderWidth: 0 }} /></View><View style={styles.liveRunnerPlayerTag}><Text style={styles.liveRunnerPlayerTagText} numberOfLines={1}>{batterLabel}</Text></View></View>
      </ImageBackground>
      {runnerActionRail}
    </View>
...
function NewGameModal({ visible, form, teams, games, onChange, onCreateTeam, onClose, onSubmit }: { visible: boolean; form: NewGameForm; teams: Team[]; games: Game[]; onChange: (form: NewGameForm) => void; onCreateTeam: (name: string) => Team | null; onClose: () => void; onSubmit: () => void }) {
  ...
  function renderGameWizard() {
    ...
    const defenseStage = (side: "away" | "home", team?: Team) => {
      ...
      return <View style={[styles.registrationPanel, styles.wizardDefensePanel, side === "home" ? styles.wizardDefenseHomePanel : styles.wizardDefenseAwayPanel]}>
        <View style={styles.wizardDefenseHeader}>
          <View style={styles.wizardDefenseCopy}><Text style={styles.registrationTitle}>{side === "home" ? "主場" : "客場"}守備位置</Text><Text style={styles.registrationHint}>直接拖拉下方球員卡到目標守位；點選卡片後也可指定。後備不列入九人先發與棒次，紅色提示代表守位重複。</Text></View>
    ...
    const battingStage = (side: "away" | "home", team?: Team) => {
      ...
      return <View style={styles.registrationPanel}>
        <Text style={styles.registrationTitle}>{side === "home" ? "主場" : "客場"}棒次排定</Text>
  ...
  const wizardStageTitle = ["盃賽名稱", "年齡層級", "正規局數", "比賽場地", "天氣", "主場球隊", "主場登錄球員", "主場守備位置", "主場排定棒次", "客場球隊", "客場登錄球員", "客場守備位置", "客場排定棒次", "主客先發確認"][wizardStep - 1] ?? "新增比賽";
  const wizardStageHint = ["輸入本場的盃賽或聯賽名稱。", "選擇本場適用的正式年齡組別。", "選擇正規局數；平手時會自動延長。", "輸入比賽場地及日期。", "選擇本場天候。", "套用既有球隊，或立即建立主場球隊。", "確認主場本場可出賽名單。", "為主場先發九人指派守備位置。", "依背號指派主場第 1 至第 9 棒。", "套用既有球隊，或立即建立客場球隊。", "確認客場本場可出賽名單。", "為客場先發九人指派守備位置。", "依背號指派客場第 1 至第 9 棒。", "送出前核對兩隊先發名單、棒次與守備位置圖。"][wizardStep - 1] ?? "";
...
const styles = StyleSheet.create({
  ...
  liveRunnerCross: { flex: 1, minWidth: 0, minHeight: 266, aspectRatio: 612 / 535, justifyContent: "center", gap: 2, overflow: "hidden", borderRadius: 9, backgroundColor: BRAND.white, borderWidth: 1, borderColor: BRAND.line },
  liveRunnerCrossBackground: { opacity: 0.85, borderRadius: 9, backgroundColor: BRAND.white },
